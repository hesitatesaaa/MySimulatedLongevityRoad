...
> 当前模组版本为 **0.2.0**。本版本让洞天、遗迹、天地之变和宗门成为可定位、可前往、可争夺并能改变区域修炼效率的山河节点。

WorldBox 模组，作者：溪上翁、kinght。模组版本 `0.2.0`，`mod.json` 中的目标游戏构建为 `115`。
...
脚本会按现有发布包的源码型结构，将 `InterestingTrait.cs`、解决方案/项目文件、`mod.json`、入口资源、`code`、`GameResources`、`Locales`、配置和说明文档放入 `发布包/`。包名格式为 `0.5.1+我的模拟长生路0.2.0-综合更新.zip`；同名时追加时间戳。该流程不要求本机存在 WorldBox 编译依赖；`references/`、构建缓存、本地 `DeveloperTools/` 和打包脚本不进入玩家发布包。
...
- 洞天、遗迹和天地之变统一为可定位的山河节点，支持生命周期、资源余量、控制者、地图标记和旧档迁移。
- 修士通过原版寻路前往节点，抵达后才执行探索、争夺和控制；空间任务具备上限、帧预算、死亡中断与降级策略。
- 增加宗门最小实体、附近节点区域修炼反馈，以及玄黄仙录“山河节点”页签和定位入口。
