...
WorldBox 模组，作者：溪上翁、kinght。模组版本 `0.1.6`，`mod.json` 中的目标游戏构建为 `115`。
...
功能页中的“进入还真空间”使用 `GameResources/ui/Icons/HuanZhenEntrance.png`。空间灵蕴按经过年份和持有者境界惰性计算，不产生逐年写盘；存在安全锚点后可每 20 年消耗 20 灵蕴进行一次轻量万界推演。推演不会创建或加载另一张地图，而是根据世界、宿主与年份生成有界摘要和少量修行收益，避免复制鬼谷模拟器的多存档高负载路径。最多保留 20 次推演记录。
