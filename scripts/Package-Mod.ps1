...
function Test-SourcePackageContents {
    ...
    try {
        ...
        foreach ($relativePath in $required) {
            if ($entries -notcontains ($root + $relativePath)) {
                throw "发布包缺少必需文件：$relativePath"
            }
        }

        foreach ($directory in @('code/', 'GameResources/', 'Locales/')) {
            if (-not ($entries | Where-Object { $_.StartsWith($root + $directory, [System.StringComparison]::OrdinalIgnoreCase) })) {
        ...
        if ($forbidden) {
            throw "发布包包含禁止内容：$($forbidden -join ', ')"
        }
        ...
        if ([string]$metadata.version -ne $Version) {
            throw "发布包中的 mod.json 版本不匹配：$($metadata.version)，期望 $Version"
        }
    }
    finally {
        $archive.Dispose()
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($version) -or $version -notmatch '^\d+\.\d+\.\d+$') {
        throw "mod.json 中的 version 不是有效的三段版本号：$version"
    }
    ...
    foreach ($item in $runtimeItems) {
        Copy-PackageItem -RelativePath $item -DestinationRoot $packageRoot
    }
    ...
    if (Test-Path -LiteralPath $zipPath) {
        ...
        do {
            if ($suffix -eq 0) {
                ...
                $collisionSuffix = "-" + [string]$suffix
            }
            $zipFileName = "{0}-{1}{2}.zip" -f $packageFolderName, $stamp, $collisionSuffix
            $zipPath = Join-Path $packageDirectory $zipFileName
            $suffix++
        } while (Test-Path -LiteralPath $zipPath)
    }

    # 使用已解析的顶层条目，兼容 Windows PowerShell 5.1 对嵌套通配符表达式的解析。
    ...
    $success = $true
    Write-Host "源码型 Mod 包已生成：$zipPath"
    ...
    if (-not $success -and $createdZip -and $zipPath -and (Test-Path -LiteralPath $zipPath)) {
